...
const normalizeLoader =
normalize(value => is(String, (value)) ? {loader: value} : value)

const normalizeLoaders =
pipe(
  ...
  addIndex(reduce)((acc, item, index, items) => {
    ...
    isNil(currentLoader) && index === 0
      ? item
      : reduce(mergeDeepLeft, item,
        takeWhile(({loader}) => loader === currentLoader || isNil(loader),
          drop(index + 1, items)))

    const shouldAppend =
      index === 0 ||
        (!isNil(currentLoader) &&
          isNil(get(findWhereEq({loader: currentLoader}), acc)))
