...
const env =
curry((defaultValue, propName) => process.env[propName] || defaultValue)

const modeFromEnv =
key => test(/^prod.*/i, env('development', key)) ? 'production' : 'development'

const mode =
modify([props('mode', 'modeEnvKey'), valueOr({})],
  cond([
...
const devtool =
modify([props('devtool', 'mode'), valueOr({})], cond([
...
const fromConfigureOptions =
opts => applyPartials(opts, [
  mode,
  devtool
])

const configure =
curry((partials, options) => applyPartials(options, [fromConfigureOptions].concat(partials)))
